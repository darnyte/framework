...
@see @ref page_ex01
*/
#ifndef __EXAMPLES_S01_HPP_
#define __EXAMPLES_S01_HPP_

#include <DeviceHive/cloud6.hpp>
#include <Examples/s00.hpp>
...
*/
class Application:
    public s00::Application
{
    typedef s00::Application Base; ///< @brief The base type.
protected:
...
{
    { // configure logging
        log::Target::SharedPtr log_file = log::Target::File::create("s01.log");
...
The full source code available in Examples/s01.hpp file.
...
Application is a console program and it's represented by an instance of s01::Application.
This instance can be created using s01::Application::create() factory method.
Application does all its useful work in s01::Application::run() method.
...
Application contains the following key objects:
    - s01::Application::m_ios the IO service instance to perform various
    asynchronous operations (such as HTTP requests and update timers)
    - s01::Application::m_device the DeviceHive device object which contains several equipment objects:
        - any number of s01::LedControl instances
        - any number of s01::TempSensor instances
    - s01::Application::m_api the "clue" which links our application and the DeviceHive server
...
We bind s01::LedControl instance with a *device* file. Once we've got
command from the server to change the LED state, this state
will be immediately written to the corresponding *device* file
using s01::LedControl::setState() method, which is very simple:

@snippet Examples/s01.hpp LedControl_setState
...
and s01::TempSensor instance reads it periodically. The *device* file format is not
...
@snippet Examples/s01.hpp TempSensor_getValue
...
At the start we should register our device on the server using s01::Application::asyncRegisterDevice() method.
If registration is successful we start listening for the commands from the server using s01::Application::asyncPollCommands() method.
At the same time we start *update* timer and periodically check the temperature sensors using s01::Application::asyncUpdateSensors().
...
@example Examples/s01.hpp
*/

#endif // __EXAMPLES_S01_HPP_
